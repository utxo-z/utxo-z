...
// =============================================================================
// Compaction with multiple version files (regression tests for dangling
// reference bugs in compact_container — see segfault at block 930000)
// =============================================================================

TEST_CASE("Compaction: survives 3+ rotations without crash", "[storage][compaction][regression]") {
    // This test triggers the dangling thdr/shdr bug in compact_container.
    // With 4+ files where all are full, the target fills up immediately on each
    // rotation attempt, causing writes to a dangling reference (unmapped memory).
    //
    ...
    // This is the line that crashes with the dangling reference bug
    db.compact_all();
...
TEST_CASE("Compaction: data integrity with many versions + close/reopen", "[storage][compaction][persistence][regression]") {
    // Triggers the same dangling reference bug, then verifies data survives
...
TEST_CASE("Compaction: with deletions across multiple versions", "[storage][compaction][regression]") {
    // Exercises compaction where some entries have been deleted from older
    // versions, creating partially-filled files. This tests the shdr dangling
    // reference path (source file empties during compaction).
    ScopedTestDir dir;
    ...
    // during compaction, triggering the shdr dangling reference path
    db.compact_all();
...
TEST_CASE("Compaction: size() is correct after compaction", "[storage][compaction][regression]") {
    // Verifies that entries_count_ (db.size()) is not double-counted after
    // compaction, which happens because open_or_create_container adds
    // entry_count to entries_count_ without subtracting the old value.
    ScopedTestDir dir;
