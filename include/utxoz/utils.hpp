 ...
 */
[[nodiscard]] std::string key_to_string(key_t const& key);
 ...
 */
[[nodiscard]] key_t make_key(std::span<uint8_t const, 32> tx_hash, uint32_t output_index);
 ...
 */
[[nodiscard]] std::span<uint8_t const, 32> get_tx_hash(key_t const& key);
 ...
 */
[[nodiscard]] uint32_t get_output_index(key_t const& key);
 ...
 */
[[nodiscard]] size_t calculate_optimal_buckets(size_t element_count, float max_load_factor = 0.88f);
