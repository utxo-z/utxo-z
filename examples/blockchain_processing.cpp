...
void init_log_file(const std::string& benchmark_name) {
    ...
    std::stringstream ss;
    ss << "blockchain_processing_" << benchmark_name << "_" 
    ...
    if (log_file.is_open()) {
        ...
        log_file << "Log started at: " << timestamp << std::endl;
        std::cout << "Benchmark: " << benchmark_name << std::endl;
        log_file << "Benchmark: " << benchmark_name << std::endl;
    }
}
...
void close_log_file() {
    if (log_file.is_open()) {
        ...
        log_file << "Log completed." << std::endl;
        log_file.close();
    }
}

template<typename... Args>
void log_print(const std::string& format, Args&&... args) {
    // Simple format replacement (basic version)
    std::string result = format;
    // For this example, we'll just print directly
    std::cout << result;
    if (log_file.is_open()) {
        log_file << result;
        log_file.flush();
...
std::vector<mock_transaction> generate_mock_block(size_t tx_count, uint32_t block_height) {
...
int main() {
    std::cout << "UTXO Database Blockchain Processing Example\n";
    std::cout << "==========================================\n\n";
    ...
    try {
        ...
        log_print("Database opened with size: " + std::to_string(db.size()) + "\n");
        ...
        for (uint32_t block_height = 0; block_height < num_blocks; ++block_height) {
            log_print("Processing block " + std::to_string(block_height) + "...\n");
            ...
            log_print("Block " + std::to_string(block_height) + " analysis:\n");
            log_print("  Regular outputs: " + std::to_string(to_insert.size()) + "\n");
            log_print("  OP_RETURN outputs: " + std::to_string(op_returns.size()) + "\n");
            log_print("  External deletes: " + std::to_string(to_delete.size()) + "\n");
            log_print("  In-block spends: " + std::to_string(in_block_spends) + "\n");
            
            // Delete UTXOs first
            for (auto const& key : to_delete) {
                db.erase(key, block_height);
                ++total_deletions;
            }
            ...
            for (auto const& [key, output] : to_insert) {
                ...
                db.insert(key, data, block_height);
                ++total_insertions;
            }
            ...
            if (block_height % 5 == 0) {
                ...
                log_print("  Deleted: " + std::to_string(deleted) + "\n");
                log_print("  Failed: " + std::to_string(failed.size()) + "\n");
            ...
            // Print progress
            if (block_height % 5 == 0) {
                log_print("=== Statistics after block " + std::to_string(block_height) + " ===\n");
                db.print_statistics();
            }
        }
        ...
        log_print("Final deleted: " + std::to_string(final_deleted) + "\n");
        log_print("Final failed: " + std::to_string(final_failed.size()) + "\n");
        ...
        log_print("\nSummary:\n");
        log_print("  Blocks processed: " + std::to_string(num_blocks) + "\n");
        log_print("  Total insertions: " + std::to_string(total_insertions) + "\n");
        log_print("  Total deletions: " + std::to_string(total_deletions) + "\n");
        log_print("  Final UTXO count: " + std::to_string(stats.total_entries) + "\n");
        log_print("  Cache hit rate: " + std::to_string(stats.cache_hit_rate * 100) + "%\n");
        ...
        log_print("Error: " + std::string(e.what()) + "\n");
        close_log_file();
        return 1;
    }
